...
cdef class MinuteSimulationClock:
    cdef bool minute_emission, daily
    ...
    def __init__(self,
        ...
        # 🆗 增加二个输入参数计算分钟，daily 简化计算
        self.am_end_nanos = am_closes.values.astype(np.int64)
        self.pm_start_nanos = pm_opens.values.astype(np.int64)
